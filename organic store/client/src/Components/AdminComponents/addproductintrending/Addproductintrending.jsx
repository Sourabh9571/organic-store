...
import { getProducts } from "../../../Slices/adminItemsSlice/adminProductsSlice.js";
import Item from "../../AdminItem/Item";
import { shownontrendingproducts } from "../../../Slices/trendingSlice/trendingSlice.js";

function Addproductintrending() {
  const dispatch = useDispatch();

  const about = "Add in Trending";

  const { loading, error, trendingproducts } = useSelector(
  ...
  useEffect(() => {
    ...
    dispatch(shownontrendingproducts());
  }, []);
  ...
  useEffect(() => {
    ...
    for (
      let i = currentIndex;
      i < trendingproducts.length && i < currentIndex + 5;
      i++
    ) {
      arr.push(trendingproducts[i]); // Access elements directly by index
    }
    setMyProducts(arr);
  ...
  const nextHandle = () => {
    setCurrentIndex(
      currentIndex + 5 > trendingproducts.length
        ? currentIndex
        : currentIndex + 5
    );
  };
      ...
      <div className="absolute w-full top-14 left-0" style={{ zIndex: "1" }}>
        ...
        <div className="flex justify-between mx-5">
          ...
          <button
            className="py-2 px-5 bg-green-400 hover:bg-green-600 hover:text-slate-200 rounded-lg"
            onClick={nextHandle}
            disabled={currentIndex + 5 >= trendingproducts.length}
          >
            Next
          </button>
        </div>
...
export default Addproductintrending;
